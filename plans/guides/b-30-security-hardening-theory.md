# B-30 — Security Hardening (теория)

> **Статус:** placeholder — наполняется по запросу.  
> **Практика:** [../backend-phase-30-security-hardening.md](../backend-phase-30-security-hardening.md)

---

## 1. Зачем эта тема

<!-- Контекст FAANG / Microsoft L63+ -->

## 2. Базовые концепции

<!-- Определения -->

## 3. Глубокое погружение

<!-- Как работает под капотом -->

## 4. Примеры кода (C#)

\\\csharp
// TODO: примеры для Security Hardening
\\\

## 5. Плюсы / минусы / когда НЕ использовать

| Плюсы | Минусы |
|-------|--------|
| | |

## 6. Сравнение с альтернативами

| Подход | Популярность | Когда выбрать |
|--------|--------------|---------------|
| | | |

## 7. Типичные ошибки

- 

## 8. Вопросы на интервью

1. 

## 9. Связь с другими фазами

- Предшествует: B-29
- Следует: B-31

## 10. Ресурсы

- [Microsoft Learn](https://learn.microsoft.com/dotnet/)
- 

